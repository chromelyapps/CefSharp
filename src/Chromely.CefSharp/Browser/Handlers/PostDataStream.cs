...
{
    /// <summary>
    /// Provides a view of a sequence of bytes in PostDataElement[] array.
    /// </summary>
    public class PostDataStream : Stream
    {
        ...
        protected IPostDataElement[] _postDataElements;
        ...
        public PostDataStream(IPostDataElement[] postDataElements)
        {
            _postDataElements = postDataElements ?? throw new ArgumentNullException(nameof(postDataElements));
            _length = _postDataElements.DefaultIfEmpty().Sum(x => (int)x.Bytes.Length);
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (bytesCopied < totalBytesToCopy)
            {
                ...
                int bytesLeftInCurrentElement = (int)postElement.Bytes.Length - _currentElementPosition;
                ...
                if (_currentElementBytes == null)
                    _currentElementBytes = postElement.Bytes;
                ...
                if (_currentElementPosition == postElement.Bytes.Length) //if we done with current element
                {
